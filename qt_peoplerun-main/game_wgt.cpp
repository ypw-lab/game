...
//    }
     gameBg[0].load(":/role/bac0.png");
     gameBg[1].load(":/role/bac2.jpg");//注意图片格式问题
    grabKeyboard();//激活键盘事件

    attckTimer.setInterval(attckInterval);//可以攻击的时限
    attckTimer.setSingleShot(true);//开启定时器后执行一次就结束

    beginTimer.setInterval(beginInterval);//开始游戏计时器 20毫秒

    connect(&beginTimer,&QTimer::timeout,[this]()
    {
        role->move(up,down,left,right);//角色移动
...
void GameWgt::keyPressEvent(QKeyEvent *event)
{
    ...
    case Qt::Key_Q:
    {
        ...
        case Qt::Key_J://按下J键进行攻击
        {
            if(isRunning&&!isPaused&&attckTimer.isActive())
            {
                QSound::play(gongjiMusic);
                attck();
            }
            break;
        }
